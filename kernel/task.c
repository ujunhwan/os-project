...
void Kernel_task_init(void)
{
    ...
    for(uint32_t i = 0; i < MAX_TASK_NUM; i++)
    {
        ...
        //debug_printf("%u task ctx->pc addr = 0x%x\n", i, &ctx->pc);
        //debug_printf("%u task ctx->spsr addr = 0x%x\n", i, &ctx->spsr);
    }
}
...
uint32_t Kernel_task_create(KernelTaskFunc_t startFunc)
{
    ...
    //debug_printf("%u create task sp = 0x%x\n",sAllocated_tcb_index - 1 , new_tcb->sp);
...
__attribute ((naked)) void Save_context(void)
{
...
__attribute ((naked)) void Restore_context(void)
{
    // restore next task stack pointer from the text TCB
